...
public interface EquipmentRepository extends JpaRepository<Equipment, Long> {
    ...
    @Query("SELECT e FROM Equipment e WHERE LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.code) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.serialNumber) LIKE LOWER(CONCAT('%', :search, '%'))")
    ...
    List<Equipment> findByOperationalStatus(OperationalStatus status);
