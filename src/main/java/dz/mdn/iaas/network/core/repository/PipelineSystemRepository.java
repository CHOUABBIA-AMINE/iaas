...
public interface PipelineSystemRepository extends JpaRepository<PipelineSystem, Long> {
    ...
    @Query("SELECT s FROM PipelineSystem s WHERE LOWER(s.code) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(s.designationAr) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(s.designationEn) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(s.designationFr) LIKE LOWER(CONCAT('%', :search, '%'))")
