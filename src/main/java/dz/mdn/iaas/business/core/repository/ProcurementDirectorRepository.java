...
public interface ProcurementDirectorRepository extends JpaRepository<ProcurementDirector, Long> {
     ...
     */
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM ProcurementDirector p WHERE p.designationFr = :designationFr")
     ...
     */
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM ProcurementDirector p WHERE p.designationFr = :designationFr AND p.id != :id")
